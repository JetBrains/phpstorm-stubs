 ...
 *
 * Distributed tracing allows you to see the path that a request takes as it travels through a distributed system. When
 * distributed tracing is enabled, use newrelic_accept_distributed_trace_headers to accept a payload of headers. These
 * include both W3C Trace Context and New Relic distributed trace headers.
 *
 * It is possible to only accept only W3C Trace Context headers and disable the New Relic Distributed Tracing header via
 * the newrelic.distributed_tracing_exclude_newrelic_header INI setting.
 *
 ...
 * newrelic_add_custom_parameter because "custom attributes" were previously called "custom parameters.") For example,
 * you can add a customer's full name from your customer database. This attribute appears in any transaction trace that
 * results from this transaction. You can also query the Transaction event for your custom attributes.
 *
 ...
 * example, you can add a customer's full name from your customer database. This attribute appears in any span. You can
 * also query the Span for your custom attributes.
 *
 ...
 * Both static and instance methods will be instrumented if the method syntax is used, and the class name must be fully
 * qualified: it must include the full namespace if the class was defined within a namespace.
 *
 ...
 * transactions, for example in the Transactions page). Call as early as possible. This is most commonly used for cron
 * jobs or other long-lived background tasks. However, this call is usually unnecessary since the agent usually detects
 * whether a transaction is a web or non-web transaction automatically.
 *
 * You can also reverse the functionality by setting the optional flag to false, which marks a "non-web" transaction as
 * a "web" transaction.
 *
 ...
 *
 * This call disables automatic injection of the browser monitoring agent for the current transaction. Call as early as
 * possible. You can use this call to remove the JavaScript if the insertion is causing problems or if you are serving
 * pages to third-party services that do not allow JavaScript (for example, Google's accelerated mobile pages).
 *
 * Returns true if called within a New Relic transaction. Otherwise returns null if outside a transaction (for example,
 * if newrelic_end_transaction() has been called).
 *
 ...
 * the agent normally does when PHP terminates the current transaction. The most common use for this call is to improve
 * instrumentation of command line scripts that handle job queue processing. Call this method at the end of a
 * particular job, and then call newrelic_start_transaction() when a new task is pulled off the queue.
 *
 * Normally, when you end a transaction you want the agent to record the associated data. However, you can also discard
 * the data by setting $ignore to true.
 *
 ...
 *
 * Ignores the current transaction when calculating your Apdex score. This is useful when you have either very short or
 * very long transactions (such as file downloads) that can skew your Apdex score.
 *
 ...
 * headers. When Distributed Tracing is enabled, newrelic_insert_distributed_trace_headers will always insert W3C trace
 * context headers. It also, by default, inserts the New Relic Distributed Tracing header, but this can be disabled via
 * the newrelic.distributed_tracing_exclude_newrelic_header INI setting.
 *
 ...
 * from the name. Instead, use parentheses (suffix) or other symbols if needed.
 *
...
/**
 * Use these calls to collect errors that the PHP agent does not collect automatically and to set the callback for your
 * own error and exception handler.
 *
 ...
 * errors that are not handled automatically so that you can query for those errors in New Relic and view error traces,
 * you can use newrelic_notice_error.
 *
 ...
 * @since 2.6
 *
 * @param string|Throwable|Exception|int $messageOrExceptionOrCode Either an error message, an exception object, or an
 *                                                                 integer error code
 * @param string|null                    $errstr                   [optional] Required if first argument is an integer,
 *                                                                 Provide an error message that will be meaningful to
 *                                                                 you when it displays in error traces.
 * @param string|null                    $errfile                  [optional] The name of the file that the error
 *                                                                 occurred in.
 * @param int|null                       $errline                  [optional] The line number where the error occurred.
 * @param string|null                    $errcontext               [optional] An array that points to the symbol table
 ...
 * @param array  $attributes Supply custom attributes as an associative array. Supply the attribute names as keys of up
 *                           to 255 characters, and supply the values as scalar values. Arrays and objects are not
 *                           supported.
 *
 * @return void
 */
 ...
 * - product: (string) Required. The name of the datastore product being used: for example, MySQL to indicate that the
 ...
 * - query: (string) Optional. The query that was sent to the server. For security reasons, this value is only used if
 ...
 * - inputQuery: (string) Optional. The input query that was provided to the ORM. For security reasons, and as with the
 * query parameter, this value will be ignored if the product is not a supported datastore.
 *
 ...
 *
 * TIP: This call only allows you to assign values to pre-existing keys. For a more flexible method to create key/value
 * pairs, use newrelic_add_custom_parameter.
 *
 ...
 * @since 3.1.5.111
 *
 * @param string $user_value    Specify a name or username to associate with this page view. This value is assigned to
 *                              the user key.
 ...
 * @param string $product_value Specify the name of a product to associate with this page view. This value is assigned
 *                              to the product key.
 *
 * @return bool
 */
 ...
 * @since 3.0.5.95
 *
 * @param string      $appname The application name to associate with data from this transaction. Uses the same format
 ...
 *                             Provide a different license key if you want the transaction to report to a different New
 *                             Relic account. If set, this license will supersede all per-directory and global default
