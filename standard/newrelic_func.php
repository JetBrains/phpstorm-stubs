<?php
/**
 * PHPStorm stub file for New Relic functions.
 *
 ...
 *
 * Warning: If you are using your custom parameters/attributes in Insights, avoid using any of Insights' reserved
 * words for naming them.
 *
 ...
 *
 * If the flag argument is set to true or omitted, the current transaction is marked as a background job. If flag
 * is set to false, then the transaction is marked as a web transaction.
 *
 ...
 * with a Custom/ prefix (for example, Custom/MyMetric). This will make them easily usable in custom dashboards. If
 * the value is NaN, Infinity, denorm or negative zero, the behavior of this function is undefined. New Relic may
 * discard 1 or more bits of precision (ULPs) from the given value.
 *
 * This function will return true if the metric was added successfully.
 *
 * Warning: Avoid creating too many unique custom metric names. New Relic limits the total number of custom metrics
 * you can use (not the total you can report for each of these custom metrics). Exceeding more than 2000 unique
 * custom metric names can cause automatic clamps that will affect other data.
 *
 ...
 * referred to as real user monitoring or RUM).
 *
 ...
 * footer for page load timing (sometimes referred to as real user monitoring or RUM) has been sent. Avoid creating
 * too many unique transaction names. This will make your graphs less useful, and you may run into limits we set on
 * the number of unique transaction names per account. It also can slow down the performance of your application.
 *
 ...
 *
 * With the second form of the call, only the message is used. This set of parameters allows newrelic_notice_error
 * to be set as an error handler with the internal PHP function set_error_handler(). With the second form of the
 * call, only the message is used.
 *
 ...
 *
 * For more information, see Inserting custom events with the PHP agent. The attributes parameter is expected to be
 * an associative array: the keys should be the attribute names (which may be up to 255 characters in length), and
 * the values should be scalar values: arrays and objects are not supported.
 *
 ...
 * that setting. The first application name is the primary name. You can also specify up to two extra application
 * names. (However, the same application name can only ever be used once as a primary name.) Call this function as
 * early as possible. It will have no effect if called after the JavaScript footer for page load timing (sometimes
 ...
 * parameter is false.
 *
 * Consider setting the application name in a file loaded by PHP's auto_prepend_file  INI setting. This function
 ...
 * browser traces. All three parameters are required, but may be empty strings. * This function will return true if
 * the attributes were added successfully.
 *
 ...
 * occur when the script was first started. Of the two arguments, only the application name is mandatory. However,
 * if you are processing tasks for multiple accounts, you may also provide a license for the associated account.
 * The license set for this API call will supersede all per-directory and global default licenses configured in INI
 * files.
 *
