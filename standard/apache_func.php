<?php
/**
 * PHPStorm stub file for Apache functions.
 *
 * @link  http://php.net/manual/en/book.apache.php
 */

/**
 * Terminate apache process after this request
 * apache_child_terminate() will register the Apache process executing the current PHP request for termination once
 * execution of PHP code is completed. It may be used to terminate a process after a script with high memory
 * consumption has been run as memory will usually only be freed internally but not given back to the operating
 * system.
 *
 * @link  http://php.net/manual/en/function.apache-child-terminate.php
 * @return bool <b>TRUE</b> if PHP is running as an Apache 1 module, the Apache version is non-multithreaded, and
 *              the child_terminate PHP directive is enabled (disabled by default). If these conditions are not
 *              met,
 *              <b>FALSE</b> is returned and an error of level <b>E_WARNING</b> is generated.
 * @since 4.0.5
 * @since 5.0
 */
 ...
 * the given resource. This function is supported when PHP is installed as an Apache module or by the NSAPI server
 * module in Netscape/iPlanet/SunONE webservers.
 *
 ...
 * This function is a wrapper for Apache's table_get and table_set. It edits the table of notes that exists during
 * a
 * request. The table's purpose is to allow Apache modules to communicate. The main use for apache_note() is to
 * pass
 ...
 *                           </p>
 *
 * @return string|false If called with one argument, it returns the current value of note note_name. If called with
 *                      two arguments, it sets the value of note note_name to note_value and returns the previous
 *                      value of note note_name. If the note cannot be retrieved, <b>FALSE</b> is returned.
 * @since 4.0
 * @since 5.0
 */
 ...
 * @return array|false An associative array of all the HTTP headers in the current request, or <b>FALSE</b on
 *                     failure.
 * @since 4.3.0
 * @since 5.0
 */
 ...
 * apache_reset_timeout() resets the Apache write timer, which defaults to 300 seconds. With set_time_limit(0);
 * ignore_user_abort(true) and periodic apache_reset_timeout() calls, Apache can theoretically run forever. This
 * function requires Apache 1.
 *
 ...
 * This function is an alias for apache_request_headers(). Please read the apache_request_headers() documentation
 ...
 * you would parse through Apache. Note that for a CGI script, the script must generate valid CGI headers. At the
 * minimum that means it must generate a Content-Type header. To run the sub-request, all buffers are terminated
 * and flushed to the browser, pending headers are sent too. This function is supported when PHP is installed as an
 * Apache module or by the NSAPI server module in Netscape/iPlanet/SunONE webservers.
 *
