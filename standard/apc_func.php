<?php
/**
 * PHPStorm stub file for APC functions.
 *
 ...
 *
 * @param string[]|null $files     The files. Passing in NULL signals a dump of every entry, while passing in
 *                                 array() will dump nothing.
 * @param string[]|null $user_vars The user vars. Passing in NULL signals a dump of every entry, while passing in
 *                                 array() will dump nothing.
 *
 * @return string|bool|null Returns a binary dump of the given files and user variables from the APC cache, FALSE
 *                          if
 ...
 * @param int           $flags     Flags passed to the filename stream. See the file_put_contents() documentation
 *                                 for details.
 * @param resource      $context   The context passed to the filename stream. See the file_put_contents()
 ...
 *                        bytecode cache for the current request will be returned. This feature must be enabled at
 *                        compile time using --enable-filehits. If an invalid or no cache_type is specified,
 ...
 *                               The constant_name must follow the normal constant naming rules. Value must
 *                               evaluate to a scalar value.
 * @param bool   $case_sensitive The default behaviour for constants is to be declared case-sensitive;
 *                               i.e. CONSTANT and Constant represent different values. If this parameter evaluates
 *                               to FALSE the constants will be declared as case-insensitive symbols.
 *
 * @return bool Returns TRUE on success or FALSE on failure.
 */
 ...
 * @param int          $ttl [optional]  Time To Live; store var in the cache for ttl seconds. After the ttl has
 *                          passed, the stored variable will be expunged from the cache (on the next request). If
 *                          no ttl is supplied
 *                          (or if the ttl is 0), the value will persist until it is removed from the cache
