<?php
/**
 * PHPStorm stub file for XDebug functions.
 */

/**
 * This function makes the debugger break on the specific line as if a normal file/line breakpoint was set on this
 * line.
 *
 * @return bool
 */
...
/**
 * This function displays structured information about one or more variables that includes its type, value and
 * refcount information. Arrays are explored recursively with values. This function is implemented differently from
 * PHP's debug_zval_dump() function in order to work around the problems that that function has because the
 * variable itself is actually passed to the function. Xdebug's version is better as it uses the variable name to
 * lookup the variable in the internal symbol table and accesses all the properties directly without having to deal
 * with actually passing a variable to a function. The result is that the information that this function returns is
 * much more accurate than PHP's own function for showing zval information.
 *
 ...
 * but directly shown on stdout (so that when you run it with apache in single process mode it ends up on the
 * console).
 *
 * @param string $varName
 */
...
/**
 * This function returns all errors from the collection buffer that contains all errors that were stored there when
 * error collection was started with xdebug_start_error_collection(). By default this function will not clear the
 * error collection buffer. If you pass true as argument to this function then the buffer will be cleared as well.
 ...
 * The information that is collected consists of an two dimensional array with as primary index the executed
 * filename and as secondary key the line number. The value in the elements represents the total number of
 * execution units on this line have been executed. Options to this function are: XDEBUG_CC_UNUSED Enables scanning
 * of code to figure out which line has executable code. XDEBUG_CC_DEAD_CODE Enables branch analyzes to figure out
 ...
 * displayed.
 * This is really useful if you want to prevent Xdebug's powerful error reporting features from destroying your
 * layout.
 */
 ...
 * Function monitoring allows you to find out where in your code the functions that you provided as argument are
 * called from. This can be used to track where old, or, discouraged functions are used. The defined functions are
 * case sensitive, and a dynamic call to a static method will not be caught.
 *
 ...
 * If no filename is given, then the trace file will be placed in the directory as configured by the
 * xdebug.trace_output_dir setting. In case a file name is given as first parameter, the name is relative to the
 * current working directory. This current working directory might be different than you expect it to be, so please
 * use an absolute path in case you specify a file name. Use the PHP function getcwd() to figure out what the
 ...
 * The errors stored in the collection buffer are not deleted and still available to be fetched through
