 ...
 * @param  $ttl   [optional] int time to live in seconds. ZendCache keeps the objects in the cache as long as the
 *                TTL is no expired, once expired it will be removed from the cache
 *
 ...
 * @param  $key   string the data's key. Possibly prefixed with namespace
 * @param  $value mixed can be any PHP object that can be serialized.
 * @param  $ttl   int  [optional] time to live in seconds. ZendCache keeps the objects in the cache as long as the
 *                TTL is no expired, once expired it will be removed from the cache
 *
 * @return boolean FALSE when stored failed, TRUE otherwise
 */
