 ...
 * This function should be called as early as possible, and will have no effect if called after the RUM footer has been
 * sent. You may want to consider setting the application name in a file loaded by PHP's auto_prepend_file INI setting.
 *
 * If you use multiple licenses you can also specify a license key along with the application name.
 * An application can appear in more than one account and the license key controls which account you are changing the
 * name in. If you do not wish to change the license and wish to use the third variant, simply set the license key to
 * the empty string ("").
 *
 ...
 * and should be used for reporting exceptions. Only the exception for the last call is retained during the course of a
 * transaction. The exception parameter must be a valid PHP Exception class, and the stack frame recorded in that class
 * will be the one reported, rather than the stack at the time this function was called. When using this form, if the
 * error message is empty, a standard message in the same format as created by Exception::__toString() will be
 * automatically generated.
 *
 * Note: With the: newrelic_notice_error($message, Exception $ex = null, $unused_1 = null, $unused_2 = null, $unused_3
 * = null) form of the call, only the $ex is used. This form of the call was designed to be a valid function passed to
 * the PHP internal function set_error_handler().
 *
 * @link https://newrelic.com/docs/php/the-php-api
 *
 ...
 *
 * This can be useful if you have implemented your own dispatching scheme and wish to name transactions according to
 ...
 * This will make your graphs less useful, and may run into limits we set on the number of unique transaction names per
 * account. It also can slow down the performance of your application. Instead, store the transaction as /product/*, or
 * use something significant about the code itself to name the transaction, such as /Product/view. The limit for the
 * total number of transactions should be less than 1000 unique transaction names -- exceeding that is not recommended.
 *
 * @link https://newrelic.com/docs/php/the-php-api
 *
 * @param string $name
 ...
 * (file download, audio or video streaming etc) and you don't want the time taken to stream to be counted as part of
 * the transaction. This is especially relevant when the time taken to complete the operation is completely outside the
 * bounds of your application. For example, a user on a very slow connection may take a very long time to download even
 * small files, and you wouldn't want that download time to skew the real transaction time.
 *
 * @link https://newrelic.com/docs/php/the-php-api
 *
 */
 ...
 * You would use this call at the end of processing a single job task, and begin a new transaction when a new task is
 ...
 * that occur when the script was first started.
 *
 * However, if you are processing tasks for multiple accounts, you may also provide a license for the associated
 * account. The license set for this API call will supersede all per-directory and global default licenses configured
 * in INI files.
 *
 * @link https://newrelic.com/docs/php/the-php-api
 *
 ...
 *  New Relic limits the total number of custom metrics you can use (not the total you can report for each of these
 * custom metrics). Exceeding more than 2000 unique custom metric names can cause automatic clamps that will affect
 * other data.
 *
 * Your custom metrics can then be used in custom dashboards and custom views in the New Relic user interface.
 * It's a best practice to name your custom metrics with a Custom/ prefix. This will make them easily usable in custom
 * dashboards.
 *
 * @link https://newrelic.com/docs/php/the-php-api
 *
